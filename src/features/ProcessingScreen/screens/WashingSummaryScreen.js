// src/features/ProcessingScreen/screens/WashingSummaryScreen.js
// Summary/List screen for washing records
...
import { syncAllWashingRecords } from '../../../services/washingService';
    ...
    return (
        <View style={styles.detailViewContainer}>
            <View style={styles.detailHeader}>
                ...
                <Text style={styles.detailHeaderTitle}>Washing Details</Text>
                <View style={{ width: 40 }} />
            </View>

            <ScrollView style={styles.detailScrollView} contentContainerStyle={styles.detailContent}>
                <View style={styles.detailNameCard}>
                    <Ionicons name="water-outline" size={32} color={CoffeeColors.ACCENT} style={{ marginBottom: 8 }} />
...
export default function WashingSummaryScreen({ navigation }) {
    ...
    const loadRecords = useCallback(async () => {
        ...
        try {
            ...
            if (storedData) {
                ...
                console.log('[WashingSummary] Loaded', parsedRecords.length, 'records,', unsynced, 'unsynced');
            } else {
                setRecords([]);
                setUnsyncedCount(0);
    ...
    useEffect(() => {
        const unsubscribe = navigation.addListener('focus', () => {
            console.log('[WashingSummary] Screen focused, refreshing data...');
            loadRecords();
        });
        return unsubscribe;
    ...
    const handleSync = async () => {
        ...
        if (unsyncedCount === 0) {
            setAlertConfig({
                ...
                message: 'All washing records are already synced.',
                type: 'info',
        ...
        setIsSyncing(true);
        try {
            const result = await syncAllWashingRecords();
            ...
            if (result.totalCount === 0) {
                ...
                setAlertConfig({
                    ...
                    message: `All ${result.syncedCount} washing records have been synced.`,
                    type: 'success',
                    buttons: [{ text: 'OK', onPress: () => setAlertVisible(false) }]
                });
                setAlertVisible(true);
            ...
            console.error('[WashingSummary] Sync error:', error);
            setAlertConfig({
                title: 'Sync Failed',
                message: error.message || 'Failed to sync washing records.',
                type: 'error',
                buttons: [{ text: 'OK', onPress: () => setAlertVisible(false) }]
            });
            setAlertVisible(true);
        ...
        >
            ...
            <View style={styles.recordActions}>
                <TouchableOpacity
                    ...
                    onPress={(e) => {
                        e.stopPropagation();
                        navigation.navigate('WashingForm', { editData: item });
                    }}
                >
    ...
    if (isLoading) {
        return (
            <View style={styles.loadingContainer}>
                ...
                <Text style={styles.loadingText}>Loading washing records...</Text>
            </View>
        );
    }
                    ...
                    {viewMode === 'list' && (
                        <>
                            <TouchableOpacity
                                ...
                                onPress={() => navigation.navigate('WashingForm')}
                            >
                                <Ionicons name="add-circle" size={20} color={CoffeeColors.WHITE} />
                                <Text style={styles.addButtonText}>Add New Washing Record</Text>
                            </TouchableOpacity>
                            ...
                            <FlatList
                                ...
                                ListEmptyComponent={
                                    <View style={styles.emptyContainer}>
                                        ...
                                        <Text style={styles.emptyText}>No washing records found</Text>
