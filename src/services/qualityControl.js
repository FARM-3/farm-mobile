 ...
 * Frontend generates grade_id in format: {HARVEST_ID}-GR{A/B}
 * Backend auto-fills ripeness_score from related Ripeness test
 *
 ...
 * @returns {Promise<Object>} Created floating test with frontend-generated grade_id and auto-filled ripeness_score
 */
export const addFloatingRecord = async (floatingData) => {
  try {
    console.log('[QualityControl] Creating floating test:', floatingData);

    // Generate grade_id: {HARVEST_ID}-GR{A/B}
    const gradeCode = floatingData.grade.toUpperCase()[0]; // 'A' or 'B'
    const gradeId = `${floatingData.harvest_id}-GR${gradeCode}`;

    const payload = {
      grade_id: gradeId, // Frontend-generated
      harvest: floatingData.harvest_id,
      grade: floatingData.grade,
      weight: parseFloat(floatingData.weight),
      date: floatingData.date
      // Note: ripeness_score is auto-filled by backend
    };

    console.log('[QualityControl] Generated grade_id:', gradeId);
